...
void CSoftKeyboard::SwitchInputStyle()
{
    ...
    case eInputStyle_EN:
        ...
        break;
    case eInputStyle_CH:
        ui->control_Language->setText(QTextCodec::codecForName("System")->toUnicode("中"));
        break;
    default: break;
    }
}
...
float CSoftKeyboard::setPointX()
{
	QWidget* pWidget = dynamic_cast<QWidget*>(m_CallObj);
	qDebug() << "pos is " << pWidget->pos();
	qDebug() << "width is " << pWidget->rect().width();
	qDebug() << "height is " << pWidget->rect().height();
	qDebug() << "show pos is " << pWidget->mapToGlobal(pWidget->pos());
	return pWidget->mapToGlobal(pWidget->pos()).x();
}
float CSoftKeyboard::setPointY()
{
	QWidget* pWidget = dynamic_cast<QWidget*>(m_CallObj);

	return pWidget->mapToGlobal(pWidget->pos()).y();
}
...
void ShowKeyboard()
{
	...
	//CSoftKeyboard::GetInstance().move((QApplication::desktop()->width() - CSoftKeyboard::GetInstance().width()) / 2, (QApplication::desktop()->height() - CSoftKeyboard::GetInstance().height()) / 2);
	CSoftKeyboard::GetInstance().move(CSoftKeyboard::GetInstance().setPointX(), CSoftKeyboard::GetInstance().setPointY() );
}
